...
export const uploadFiles = async (files: UploadedFile[], folderId: string) => {
    try {
        console.log(files)
        ...
        for (const file of files) {
            ...
            if(firstFolder) {
                ...
                const firstFolderResult = await (conn as Pool).query(queryFirstFolder, [fileFolderPath[0], parentFolder, user.id])
            ...
            for (const folder of fileFolderPathWithoutFirst) {
                ...
                if (checkResult.rows.length > 0) {
                    ...
                    const insertResult = await (conn as Pool).query(insertQuery, [folder, parentFolder, user.id])
                    parentFolder = insertResult.rows[0].id
                }
            }

            const query = 'INSERT INTO files (name, path, folder_id, user_id) VALUES ($1, $2, $3, $4)'
            const result = await (conn as Pool).query(query, [file.name, file.savedPath, parentFolder, user.id])
        }
